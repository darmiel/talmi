...
func (c *Client) TaskLogs(ctx context.Context, name string) ([]tasks.LogEntry, string, error) {
	...
	correlationID, err := c.get(ctx, c.url().
		setPath(api.TaskLogsRoute).
		setPathParam("name", name).
		build(), &out)
	return out, correlationID, err
}
