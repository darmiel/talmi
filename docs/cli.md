...
Results go to **stdout** (so `--json | jq` is safe); logs, progress, and errors go to **stderr**. Errors print a
headline, hints, and the underlying cause, with a stable exit code: `2` usage,
...
| `talmi audit list`                        | Query the audit log                                                                                                     |
