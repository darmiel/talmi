...
[Layout and trust model](layout.md#three-validation-layers)). The one you run yourself is
`config vet`.
...
- **Offline** checks structure: types, cross-references (a rule pointing at a known issuer/realm),
  duplicate names, pattern and action validity, capability blocks, and realm-name defaults/collisions.
- **`--online`** builds each provider and asks what resources it can serve, then flags rules that
...
Findings carry a stable code (e.g. `CFG-XREF-REALM`, `CFG-REALM-CAP`) and a location, so you can act
on them or filter in `--json` output.
...
`make schema` after changing a config type. See [Reference > JSON schema](../reference/json-schema.md).

## Runtime

On `server run` (non-dev), Talmi runs the offline checks and refuses to start on errors, then
compiles rule expressions and validates conditions once more. `--dev` skips this gate.
