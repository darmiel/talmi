...
The bootstrap file is the trusted, host-local entry point. A JSON Schema is available for editor
...
audit:
  ...
  retention: 90d                 # 0/unset = keep forever
  connect_timeout: 10s
...
# ...local includes or a github block; see Config sources
...
Every secret-bearing value (`dsn`, `key`, provider credentials) is a [`secret.Ref`](secrets.md), not
an inline literal.
